...
DAYS_7 = 7
ALG_GENESIS_UNIXTIME = 1548690730
...
def get_reward_payments(address, after_time, session, created_at_round):
    ...
    while not done:
        params = {
            ...
            "tx-type": "pay",
            "limit": 1000,
            "min-round": created_at_round,
            "after-time": after_time.isoformat()
        }
        if next_token:
...
def process_delegator(row, after_time_7d, latest_round, session):
    ...
    if staked < 30000:
        return None
    SECONDS_PER_ROUND = 2.8
    active_rounds = latest_round - created_at_round
    active_seconds = active_rounds * SECONDS_PER_ROUND
    days_active = active_seconds / 86400

    paid_7d = sum(
        tx.get("payment-transaction", {}).get("amount", 0)
        for tx in get_reward_payments(addr, after_time_7d, session, created_at_round)
    ) / μALGO

    paid_total = sum(
        tx.get("payment-transaction", {}).get("amount", 0)
        for tx in get_reward_payments_since_round(addr, created_at_round, session)
    ) / μALGO

    period_seconds = (latest_round - created_at_round) * SECONDS_PER_ROUND
    period_seconds = min(period_seconds, 7 * 86400)

    apy_7d = (paid_7d / staked) * (365 * 86400 * 100 / period_seconds)

    apy_total = (paid_total / staked) * (365 / days_active) * 100 if staked and days_active > 0 else 0
...
def main():
    ...
    with sqlite3.connect(DB_PATH, timeout=15) as conn, requests.Session() as session:
