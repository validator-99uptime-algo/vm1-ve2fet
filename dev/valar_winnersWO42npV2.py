...
BLACKLIST_ADDRS = {
    ...
    "LVU5WJPG6DBOGTQ4L67TRBL7D3QF6KE26PEKNBO2YVBUTYT2PTMOUNNUKA",
}
...
def main() -> None:
    ...
    for stake in CHECKPOINTS:
        ...
        best_three = costs.nsmallest(3)

        for rank, idx in enumerate(best_three.index, start=1):
            row = elig.loc[idx]
            mark = "Y" if row.mine else "N"
            if rank == 1:
                label = "1st"
                stake_col = f"{stake // 1000:5d} k"
            elif rank == 2:
                label = "2nd"
                stake_col = " " * 7
            else:
            ...
            color = (
                "" if row.mine else (RED if rank == 1 else YELLOW)
            )
            print(
                f"{color}"
                f"{stake_col}  {label}  {int(row.ad_id):7d}   {mark}  {best_three[idx]:6.2f}  "
